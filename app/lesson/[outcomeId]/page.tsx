          ...
          <div className="max-w-7xl mx-auto flex items-center justify-between gap-2 flex-wrap">
            <div className="flex items-center gap-1.5 flex-wrap">
              <button
                ...
                className="hover:text-teal-700 hover:bg-slate-200/70 px-2 py-0.5 rounded-lg transition-all flex items-center gap-1 cursor-pointer"
                title="Ana Sayfaya Dön (Sınıf Seçimi)"
              >
              ...
              {pathInfo && (
                <>
                  ...
                  <button
                    onClick={() => {
                      playSound('click');
                      setSelectedGrade(pathInfo.grade);
                    ...
                    className="hover:text-teal-700 hover:bg-slate-200/70 px-2 py-0.5 rounded-lg transition-all cursor-pointer font-semibold"
                    title={`${pathInfo.grade.title} Ders Seçimine Git`}
                  >
                    {pathInfo.grade.title}
                  </button>
                  ...
                  <button
                    onClick={() => {
                      playSound('click');
                      setSelectedGrade(pathInfo.grade);
                      setSelectedSubject(pathInfo.subject);
                      setSelectedUnit(null);
                      setSelectedTopic(null);
                      setSelectedOutcome(null);
                      router.push('/');
                    ...
                    className="hover:text-teal-700 hover:bg-slate-200/70 px-2 py-0.5 rounded-lg transition-all cursor-pointer font-semibold"
                    title={`${pathInfo.subject.title} Ünite Seçimine Git`}
                  >
                    {pathInfo.subject.title}
                  </button>
                  ...
                  <button
                    onClick={() => {
                      ...
                      setSelectedGrade(pathInfo.grade);
                      setSelectedSubject(pathInfo.subject);
                      setSelectedUnit(pathInfo.unit);
                      setSelectedTopic(null);
                      setSelectedOutcome(null);
                      router.push('/');
                    }}
                    className="hover:text-teal-700 hover:bg-slate-200/70 px-2 py-0.5 rounded-lg transition-all cursor-pointer line-clamp-1 max-w-[200px] sm:max-w-none font-semibold"
                    title={`${pathInfo.unit.title} Kazanım Listesine Git`}
                  >
                    {pathInfo.unit.title.split(':')[0]}
                  </button>

                  <ChevronRight className="w-3 h-3 text-slate-400 shrink-0" />
                  <span className="text-teal-800 font-bold bg-teal-50 px-2.5 py-0.5 rounded-md border border-teal-200">
                    {outcome.code}
                  </span>
                </>
              )}
